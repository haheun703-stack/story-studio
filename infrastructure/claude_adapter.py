...
class ClaudeStoryGenerator(IStoryGenerator):
    """Claude 모델을 사용하여 동화 스토리를 생성하는 어댑터"""
    ...
    def generate_story(self, audience: TargetAudience, theme: str) -> Story:
        ...
        while len(all_scenes) < 150:
            ...
            logger.info("Claude API 호출 중... (배치 %d, 씬 %d번부터 %d개 요청)", batch_num, start_scene, request_count)

            try:
                response = self.client.messages.create(
                    model=self.model_name,
                    max_tokens=4096,
                    system=f"당신은 {audience.value} 대상 동화 작가입니다. 반드시 JSON 형식으로만 응답하세요.",
                    messages=[{"role": "user", "content": prompt}],
                )
                response_text = response.content[0].text
            except Exception as e:
                raise StoryGenerationError("Claude", str(e))
    ...
    def _generate_title(self, audience: TargetAudience, theme: str) -> str:
        try:
            response = self.client.messages.create(
                ...
                messages=[{
                    "role": "user",
                    "content": f'{audience.value} 대상 동화의 제목을 1개만 생성하세요.\n주제: "{theme}"\nJSON 형식: {{"title": "..."}}',
                }],
            )
