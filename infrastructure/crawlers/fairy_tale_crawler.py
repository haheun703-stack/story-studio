...
class FairyTaleCrawler(IContentCrawler):
    ...
    def _search_with_llm(self, topic: str) -> List[dict]:
        ...
        """
        prompt = (
            f"다음 주제에 대해 유치부(3~7세) 아이들을 위한 동화 소재를 5개 제안해주세요.\n"
            f"주제: {topic}\n\n"
            f"## 소재 선별 기준\n"
            f"- 교육적 가치: 우정, 용기, 나눔, 배려, 호기심 등 핵심 가치 포함\n"
            f"- 연령 적합성: 3~7세가 이해할 수 있는 단순한 갈등 구조\n"
            f"- 시각적 매력: 다채로운 장면 연출이 가능한 소재 (숲, 바다, 하늘, 마을 등)\n"
            f"- 캐릭터 잠재력: 매력적인 주인공과 조연을 만들 수 있는 소재\n"
            f"- 각 소재는 서로 다른 배경/분위기를 가져야 합니다\n\n"
            f"반드시 아래 JSON 배열 형식으로만 응답해주세요:\n"
            f'[{{"title": "소재 제목", "summary": "줄거리 요약 (2~3문장)", '
            f'"keywords": ["키워드1", "키워드2", "키워드3"], '
            f'"educational_value": "이 소재가 가르치는 핵심 가치"}}]'
        )
